...
export class TicketListComponent implements OnInit, OnDestroy, OnChanges {
  ticketList: Ticket[] = []
  ...
  loadTickets(): void {
    ...
    // Admin için tüm ticket'ları getir
    this.http.get<Ticket[]>(`${environment.apiUrl}/tickets`, { headers: this.headers }).subscribe({
      next: (tickets) => {
