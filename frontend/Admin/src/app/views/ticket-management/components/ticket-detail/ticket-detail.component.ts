...
import type { Ticket, TicketComment, TicketAttachment } from '../ticket-list/ticket-list.component'
...
export class TicketDetailComponent {
  ...
  updateTicketStatus(newStatus: string): void {
    if (!this.ticket?.id || this.updatingStatus) return
    ...
    this.http.put<Ticket>(`${environment.apiUrl}/tickets/${this.ticket.id}`, updateData, { headers: this.headers }).subscribe({
      next: (updatedTicket) => {
        this.ticket = updatedTicket
        this.updatingStatus = false
        console.log('Ticket status güncellendi:', newStatus)
      },
      error: (err) => {
        console.error('Status güncelleme hatası:', err)
        alert(err.error?.detail || err.error?.message || 'Status güncellenirken bir hata oluştu')
        this.updatingStatus = false
      }
    })
  }
